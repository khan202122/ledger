	...
	LOOP
		...
		VALUES(p ->> 'source', p ->> 'asset', 0, (p ->> 'amount')::bigint)
		    ON CONFLICT (account, asset) DO UPDATE SET output = (p ->> 'amount')::bigint + (
                ...
                WHERE account = p ->> 'source' AND asset = p ->> 'asset'
            );
        ...
        VALUES(p ->> 'destination', p ->> 'asset', (p ->> 'amount')::bigint, 0)
            ON CONFLICT (account, asset) DO UPDATE SET input = (p ->> 'amount')::bigint + (
                SELECT input
                FROM "VAR_LEDGER_NAME".volumes
                WHERE account = p ->> 'destination' AND asset = p ->> 'asset'
            );
        INSERT INTO "VAR_LEDGER_NAME".accounts (address, metadata) VALUES (p ->> 'source', '{}') ON CONFLICT DO NOTHING;
        INSERT INTO "VAR_LEDGER_NAME".accounts (address, metadata) VALUES (p ->> 'destination', '{}') ON CONFLICT DO NOTHING;
    END LOOP;
RETURN NEW;
END;
$$;
...
--statement
CREATE INDEX IF NOT EXISTS balances_account ON "VAR_LEDGER_NAME".balances (
    "account"
);
--statement
CREATE INDEX IF NOT EXISTS volumes_account ON "VAR_LEDGER_NAME".volumes (
    "account"
);
--statement
...
BEGIN
	if NEW.type = 'NEW_TRANSACTION' THEN
		...
		VALUES (
			(NEW.data->>'txid')::varchar,
			(NEW.data->>'timestamp')::varchar,
			CASE WHEN (NEW.data->>'reference')::varchar = '' THEN NULL ELSE (NEW.data->>'reference')::varchar END,
